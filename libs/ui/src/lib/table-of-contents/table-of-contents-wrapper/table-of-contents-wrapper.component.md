...
2. As its content, add a div with multiple blocks (eg same `div`s) of textual content with `veeraNgToCSection="section-id"` attribute each. Where `section-id` is a unique alphanumeric string in kebab case;
