  ...
  private generateLayout() {
    ...
    for (let i = 1; i < layers.length; i++) {
      layers[i].sort((a, b) => {
        ...
        const aBarycenter = aBox?.targets ? aBox.targets.reduce((acc, id) => acc + layers[i - 1].indexOf(id), 0) / aBox.targets.length : 0;
        const bBarycenter = bBox?.targets ? bBox.targets.reduce((acc, id) => acc + layers[i - 1].indexOf(id), 0) / bBox.targets.length : 0;
